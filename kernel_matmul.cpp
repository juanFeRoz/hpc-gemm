...
extern "C" float run_kernel(int M, int N, int K, int BM, int BN, int BK,
  ...
  buffer<float> A_buf(A.data(), range<1>(M * K));
  buffer<float> B_buf(B.data(), range<1>(K * N));
  buffer<float> C_buf(C.data(), range<1>(M * N));
  ...
  q.submit([&](handler &h) {
    accessor Aacc(A_buf, h, read_only);
    accessor Bacc(B_buf, h, read_only);
    accessor Cacc(C_buf, h, write_only);
                   ...
                   [=](nd_item<2> it) {
                     ...
                     for (int k0 = 0; k0 < K; k0 += BK) {
                       ...
                       for (int i = 0; i < TM; i++) {
                         ...
                         if (r < M && c < K)
                           As[threadRow * TM + i][threadCol] = Aacc[r * K + c];
                       }
                       ...
                       if (rB < K && cB < N)
                         Bs[threadRow][threadCol] = Bacc[rB * N + cB];
